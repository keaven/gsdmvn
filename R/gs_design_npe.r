...
gs_design_npe <- function(theta = .1, theta1 = NULL, info = 1, info0 = NULL, info1 = NULL,
  ...
  lowerProb <- rep(NA, K)
  ######################################################################################
  ## Confirm final effect size is > 0
  if (theta[K] <= 0) stop("gs_design_npe: final effect size must be > 0")
  ## Confirm alpha and beta length 1 and numeric
...
errbeta <- function(x = 1, K = 1, beta = .1, theta = .1, theta1 = .1, info = 1, info1 = 1, info0 = 1, binding = FALSE,
                    ...
                    test_upper = TRUE, test_lower = TRUE,
                    r = 18, tol = 1e-6){
  if (length(info1) != length(info) || length(info0) != length(info)) stop("errbeta: length of info, info1, info0 must be the same")
  return(1 -  beta -
           gs_power_npe(theta = theta, info = info * x, info1 = info1 * x, info0 = info0 * x, binding = binding,
